...
/**
 * This part of the code generates data-driven orders insights when OpenAI is not available
 * Uses real operational data to provide meaningful insights without AI
 */
function generateOrdersDataDrivenInsights(
  orders: OrderData[],
  kpis: any,
  ...
  // At-Risk Orders Priority
  if (kpis.atRiskOrders > 0) {
    const riskValue = orders
      .filter(o => o.sla_status === 'at_risk' || o.sla_status === 'breach' || o.expected_quantity !== o.received_quantity)
      .reduce((sum, o) => sum + ((o.unit_cost || 0) * o.expected_quantity), 0);
    
    insights.push({
      type: "fulfillment_risk",
      title: "Order Fulfillment Risk Management",
      description: `${kpis.atRiskOrders} orders at risk with $${Math.round(riskValue).toLocaleString()} value exposure. Immediate action required to prevent SLA breaches.`,
      severity: kpis.atRiskOrders > 20 ? "critical" : "warning",
      dollarImpact: Math.round(riskValue),
      suggestedActions: [
        "Set up automated alerts for orders approaching SLA deadlines",
        "Create supplier performance scorecards with penalty clauses",
        "Implement order prioritization workflow based on customer tier",
        "Configure escalation process for at-risk orders"
      ]
    });
  }
  
  // Delayed Shipments Analysis
  if (inboundIntelligence.delayedShipments.count > 0) {
    const delayImpact = Math.round(inboundIntelligence.valueAtRisk);
    insights.push({
      type: "delivery_performance",
      title: "Delivery Performance Optimization",
      description: `${inboundIntelligence.delayedShipments.count} delayed shipments (${inboundIntelligence.delayedShipments.percentage.toFixed(1)}%) with ${inboundIntelligence.avgDelayDays} days average delay.`,
      severity: inboundIntelligence.delayedShipments.percentage > 15 ? "critical" : "warning",
      dollarImpact: delayImpact,
      suggestedActions: [
        "Review supplier performance agreements",
        "Implement backup supplier strategies",
        "Negotiate improved delivery commitments",
        "Establish real-time shipment tracking dashboard"
      ]
    });
  }
  
  // Unfulfillable SKUs Issue
  if (kpis.unfulfillableSKUs > 0) {
    const lostSalesRisk = kpis.unfulfillableSKUs * 750; // Conservative estimate per SKU
    insights.push({
      type: "inventory_fulfillment",
      title: "Order Fulfillment Capability",
      description: `${kpis.unfulfillableSKUs} SKUs with fulfillment issues creating potential revenue loss and customer dissatisfaction.`,
      severity: kpis.unfulfillableSKUs > 10 ? "critical" : "warning",
      dollarImpact: lostSalesRisk,
      suggestedActions: [
        "Emergency reorder for unfulfillable SKUs",
  ...
  const efficiencyRate = totalOrders > 0 ? ((totalOrders - kpis.atRiskOrders) / totalOrders) * 100 : 100;
  insights.push({
    type: "operational_efficiency",
    title: "Order Processing Performance Overview",
    description: `Processing ${totalOrders} orders with ${efficiencyRate.toFixed(1)}% efficiency. ${kpis.openPOs} active purchase orders requiring management attention.`,
    severity: efficiencyRate < 90 ? "warning" : "info",
    dollarImpact: Math.round(totalOrders * 25), // Processing value estimate
    suggestedActions: [
      "Monitor order lifecycle KPIs daily",
      "Implement automated order status updates",
      "Optimize purchase order approval workflows",
      "Establish order accuracy measurement system"
    ]
  });
